...
function createSauce(req, res){
  ...
  sauce.save()
    .then(() => res.status(201).json({ message: "new Sauce created successfully"}))
    .catch(() => res.status(400).json({ error }));
}
...
function updateSauce(req, res, next) {
  if(!req.file){
    ...
    .then(() => res.status(200).json({ message: 'La sauce a été bien modifié'}))
...
function RemoveLikeOrDislike(req, res, sauce, userId){
  if(existsUserId(sauce.usersLiked, userId) !== false){
    ...
    .then(() => res.status(200).json({ message: 'Like removed successfuly!!'}))
    .catch(error => res.status(400).json({ error }));
    ...
    Sauce.updateOne({ _id: req.params.id }, {
      ...
      .then(() => res.status(200).json({ message: 'Dislike removed successfuly!!'}))
      .catch(error => res.status(400).json({ error }));
...
function updtateThing(req, res, next) {
